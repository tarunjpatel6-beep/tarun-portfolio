...
import { BCA_QUESTION_BANKS, getBCAQuestions } from "./bcaQuestions";
import { BCA_LESSON_CONTENT } from "./bcaContent";
...
const loadUser = async (name, pin) => {
  try { const snap = await getDoc(doc(db, "bca-users", userDocId(name, pin))); return snap.exists() ? snap.data() : null; } catch { return null; }
};
...
const T = {
  ...
  text: "#e6ecf7", textDim: "#6b7a96", textMuted: "#465268",
  font: "'DM Sans',sans-serif", mono: "'JetBrains Mono','SF Mono',monospace", display: "'Bebas Neue',cursive",
};

const MODULES = [
  { id: "market", title: "The Commercial Aviation Market", icon: "🌐", desc: "The duopoly, customers, the order-to-delivery cycle, backlog, and where finance sits", duration: "12 min" },
  { id: "dealstrategy", title: "Sales Campaigns & Deal Strategy", icon: "♟️", desc: "Campaign lifecycle, customer economics, negotiation levers, the deal desk, and the handoff", duration: "13 min" },
  { id: "purchaseagreement", title: "The Aircraft Purchase Agreement", icon: "📜", desc: "The PA, letter agreements, specs, and the firm/option/right structure that drives backlog", duration: "13 min" },
  { id: "pricing", title: "Pricing & Escalation", icon: "🏷️", desc: "List vs. net price, the escalation formula, concessions and credits, and the PDP ladder", duration: "14 min" },
  { id: "negotiation", title: "Negotiation & Commercial Terms", icon: "🤝", desc: "Levers, performance guarantees, liquidated damages, MFC clauses, and termination remedies", duration: "13 min" },
  { id: "commitments", title: "Commitments & Customer Financing", icon: "🛡️", desc: "Backstop financing, residual value guarantees, trade-ins, contingencies, and recording them", duration: "13 min" },
  { id: "revrec", title: "Revenue Recognition & Delivery", icon: "✈️", desc: "ASC 606 for aircraft, control transfer at delivery, contract liabilities, variable consideration", duration: "14 min" },
];
...
function RichLesson({ moduleId, onComplete }) {
  const content = BCA_LESSON_CONTENT[moduleId];
...
function PracticeMode({ moduleId, onBack }) {
  const [deck] = useState(() => getBCAQuestions(moduleId));
...
function FinalExam({ moduleId, onComplete, onBack }) {
  ...
  const [deck] = useState(() => getBCAQuestions(moduleId, EXAM_SIZE));
  const [idx, setIdx] = useState(0);
...
function KnowledgeModule({ moduleId, mastery, onComplete }) {
  ...
  const bankSize = (BCA_QUESTION_BANKS[moduleId] || []).length;
...
// MAIN
// ═══════════════════════════════════════════════════════════════════
export default function BCAModule() {
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("bca-user") || "null");
  ...
  const doLogin = async () => {
    ...
    setUser(u);
    if (data) { setCompleted(new Set(data.completed || [])); setMastery(data.mastery || {}); }
    try { localStorage.setItem("bca-user", JSON.stringify(u)); } catch {}
    setScreen("hub"); setLoading(false);
  };

  const logout = () => {
    try { localStorage.removeItem("bca-user"); } catch {}
